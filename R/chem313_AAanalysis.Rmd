...
Average_byMetalSite <- function(Site, Metal){
  ...
  mean(C$conc_blanked)
  sd(C$conc_blanked)
  CI <- qt(0.95, df=length(C$conc_blanked)-1)*sd(C$conc_blanked)/(sqrt(length(C$conc_blanked))) ##95% CI's... copy and paste when needed!
 Conc_data <- data.frame(site=Site, metal=Metal, mean(C$conc_blanked), sd(C$conc_blanked), CI)
}
